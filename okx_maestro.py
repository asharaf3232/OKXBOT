...
try:
    ...
    logging.warning(
        "Cryptography library not found. API key encryption will be disabled."
    )
...
try:
    ...
    logging.warning(
        "Library 'scipy' not found. RSI Divergence strategy will be disabled."
    )
...
# --- الوحدات المخصصة ---
from wise_man import (
    WiseMan,
    PORTFOLIO_RISK_RULES,
)  # --- [تعديل V8.1] استيراد قواعد المخاطر
...
def get_encrypted_env(var_name):
    ...
    if value.startswith("enc:") and SECRET_KEY and CRYPTO_AVAILABLE:
        try:
            ...
            logger.critical(
                f"FATAL: Failed to decrypt {var_name}. Check your SECRET_KEY and the variable's format. Error: {e}"
            )
            return None
    elif value.startswith("enc:"):
        logger.critical(
            f"FATAL: Variable {var_name} is encrypted, but SECRET_KEY is missing or cryptography library is not installed."
        )
        return None
        ...
        **copy.deepcopy(
            {
                ...
                if "adaptive" not in k and "dynamic" not in k and "strategy" not in k
            }
        ),
        "min_win_probability": 0.60,
    },
...
class BotState:
    def __init__(self):
        ...
        self.guardian_listener_task = None
...
def load_settings():
    ...
    for key, value in default_copy.items():
        if isinstance(value, dict):
            if key not in bot_data.settings or not isinstance(
                bot_data.settings[key], dict
            ):
...
def determine_active_preset():
    current_settings_for_compare = {
        k: v
        for k, v in bot_data.settings.items()
        if k in SETTINGS_PRESETS["professional"]
    }
    for name, preset_settings in SETTINGS_PRESETS.items():
        is_match = True
        for key, value in preset_settings.items():
            if (
                key in current_settings_for_compare
                and current_settings_for_compare[key] != value
            ):
                is_match = False
                break
        if is_match:
    ...
    try:
        ...
        async with aiosqlite.connect(DB_FILE) as conn:
            ...
            if "signal_strength" not in columns:
                await conn.execute(
                    "ALTER TABLE trades ADD COLUMN signal_strength INTEGER DEFAULT 1"
                )
                added_columns.append("signal_strength")
            if "close_retries" not in columns:
                await conn.execute(
                    "ALTER TABLE trades ADD COLUMN close_retries INTEGER DEFAULT 0"
                )
                added_columns.append("close_retries")
            if "last_profit_notification_price" not in columns:
                await conn.execute(
                    "ALTER TABLE trades ADD COLUMN last_profit_notification_price REAL DEFAULT 0"
                )
                added_columns.append("last_profit_notification_price")
            if "trade_weight" not in columns:
                await conn.execute(
                    "ALTER TABLE trades ADD COLUMN trade_weight REAL DEFAULT 1.0"
                )
                added_columns.append("trade_weight")
            if "trailing_sl_active" not in columns:
                await conn.execute(
                    "ALTER TABLE trades ADD COLUMN trailing_sl_active BOOLEAN DEFAULT 0"
                )
                added_columns.append("trailing_sl_active")
            if "highest_price" not in columns:
                await conn.execute(
                    "ALTER TABLE trades ADD COLUMN highest_price REAL DEFAULT 0"
                )
                added_columns.append("highest_price")
            # --- [تعديل V8.1] إضافة الأعمدة الجديدة إذا كانت مفقودة
            if "win_prob" not in columns:
                await conn.execute(
                    "ALTER TABLE trades ADD COLUMN win_prob REAL DEFAULT 0.5"
                )
                added_columns.append("win_prob")
            if "trade_size" not in columns:
                await conn.execute(
                    "ALTER TABLE trades ADD COLUMN trade_size REAL DEFAULT 15.0"
                )
                added_columns.append("trade_size")
            await conn.commit()
            if added_columns:
                logger.info(
                    f"Added missing columns to trades table: {', '.join(added_columns)}"
                )
...
async def log_pending_trade_to_db(signal, buy_order):
    ...
    try:
        async with aiosqlite.connect(DB_FILE) as conn:
            ...
            await conn.commit()
            logger.info(
                f"Logged pending trade for {signal['symbol']} with order ID {buy_order['id']}."
            )
            return True
    except KeyError as e:
        logger.critical(
            f"CRITICAL DB LOG ERROR for {signal['symbol']}: Missing key {e} in signal data. This is a critical bug.",
            exc_info=True,
        )
        return False
    except Exception as e:
...
def build_enriched_message(message_type, data):
    ...
    if message_type == "entry_alert":
        ...
        advice = (
            "احتمالية عالية، يوصى بالمراقبة."
            if win_prob > 70
            else "احتمالية متوسطة، تتطلب الحذر."
        )
        text = (
    ...
    for i in range(3):  # عدد محاولات الإرسال
        try:
            if len(text) > max_length:
                ...
                parts = [
                    text[j : j + max_length] for j in range(0, len(text), max_length)
                ]
                for part in parts:
                    await bot.send_message(
                        TELEGRAM_CHAT_ID, part, parse_mode=ParseMode.MARKDOWN, **kwargs
                    )
            ...
            if "message is too long" in str(e).lower():
                ...
                logger.error(
                    f"Telegram BadRequest (Message too long): {e}. Retrying with split."
                )
                text = (
                    text  # النص الأصلي لا يزال موجودًا، سيعاد تقسيمه في المحاولة التالية
                )
            ...
            if isinstance(e, Forbidden):  # إذا تم حظر البوت
                logger.critical(
                    "Critical Telegram error: BOT IS BLOCKED. Cannot send messages."
                )
            ...
            logger.error(
                f"Unknown Telegram Send Error: {e}. Attempt {i+1}/3.", exc_info=True
            )
...
async def safe_api_call(api_call_func, max_retries=3, delay=5):
    ...
    for attempt in range(max_retries):
        try:
            ...
            last_exception = e
            logger.warning(
                f"API call failed with network/exchange error: {str(e)}. Attempt {attempt + 1}/{max_retries}."
            )
            if "51400" in str(e):  # خطأ "Order does not exist"
                logger.warning(
                    f"Caught OKX error 51400. The order was likely already filled or canceled. Stopping retries."
                )
    ...
    try:
        async with aiosqlite.connect(DB_FILE) as conn:
            cursor = await conn.execute(
                "SELECT reason, status, pnl_usdt FROM trades WHERE status LIKE '%(%' ORDER BY id DESC LIMIT 100"
            )
        ...
        for reason_str, status, pnl in trades:
            if not reason_str or pnl is None:
                continue
        ...
        logger.info(
            f"🧠 Adaptive Mind: Analysis complete for {len(performance_data)} strategies."
        )

    except Exception as e:
        logger.error(
            f"🧠 Adaptive Mind: Failed to analyze strategy performance: {e}",
            exc_info=True,
        )
...
async def propose_strategy_changes(context: ContextTypes.DEFAULT_TYPE):
    ...
    if not settings.get("adaptive_intelligence_enabled") or not settings.get(
        "strategy_proposal_enabled"
    ):
        return
    ...
    for scanner in active_scanners:
        ...
        if (
            ...
            and perf["win_rate"] < deactivation_wr
        ):
            ...
            keyboard = [
                [
                    InlineKeyboardButton(
                        "✅ موافقة",
                        callback_data=f"strategy_adjust_approve_{proposal_key}",
                    ),
                    InlineKeyboardButton(
                        "❌ رفض", callback_data=f"strategy_adjust_reject_{proposal_key}"
                    ),
                ]
            ]
            await safe_send_message(
                context.bot, message, reply_markup=InlineKeyboardMarkup(keyboard)
            )
            return
...
async def translate_text_gemini(text_list):
    ...
    if not text_list:
        return [], True
    prompt = (
        "Translate the following English headlines to Arabic. Return only the translated text, with each headline on a new line:\n\n"
        + "\n".join(text_list)
    )
...
def get_alpha_vantage_economic_events():
    ...
    try:
        ...
        events = [
            dict(zip(header, [v.strip() for v in line.split(",")]))
            for line in lines[1:]
        ]
        high_impact_events = [
            e.get("event", "Unknown Event")
            for e in events
            if e.get("releaseDate", "") == today_str
...
def get_latest_crypto_news(limit=15):
    ...
    headlines = [
        entry.title for url in urls for entry in feedparser.parse(url).entries[:7]
    ]
...
async def get_fundamental_market_mood():
    ...
    if high_impact_events:
        return {
            "mood": "DANGEROUS",
            "reason": f"أحداث هامة اليوم: {', '.join(high_impact_events)}",
        }
    ...
    if score > 0.25:
        return {"mood": "POSITIVE", "reason": f"مشاعر إيجابية (الدرجة: {score:.2f})"}
    elif score < -0.25:
        return {"mood": "NEGATIVE", "reason": f"مشاعر سلبية (الدرجة: {score:.2f})"}
    else:
        return {"mood": "NEUTRAL", "reason": f"مشاعر محايدة (الدرجة: {score:.2f})"}
...
async def get_market_mood():
    ...
    if settings.get("btc_trend_filter_enabled", True):
        try:
            ...
            ohlcv = await safe_api_call(
                lambda: bot_data.exchange.fetch_ohlcv(
                    "BTC/USDT", "4h", limit=htf_period + 5
                )
            )
            if not ohlcv:
                return {
                    "mood": "DANGEROUS",
                    "reason": "فشل جلب بيانات BTC (API Error)",
                    "btc_mood": "UNKNOWN",
                }
            df = pd.DataFrame(
            ...
            if not is_btc_bullish:
                return {
                    "mood": "NEGATIVE",
                    "reason": "اتجاه BTC هابط",
                    "btc_mood": btc_mood_text,
                }
        except Exception as e:
            return {
                "mood": "DANGEROUS",
                "reason": f"فشل جلب بيانات BTC: {e}",
                "btc_mood": "UNKNOWN",
            }
    else:
    ...
    if settings.get("market_mood_filter_enabled", True):
        ...
        if fng is not None and fng < settings["fear_and_greed_threshold"]:
            return {
                "mood": "NEGATIVE",
                "reason": f"مشاعر خوف شديد (F&G: {fng})",
                "btc_mood": btc_mood_text,
            }
...
def analyze_momentum_breakout(df, params, rvol, adx_value):
    ...
    macd_col, macds_col, bbu_col, rsi_col = (
        find_col(df.columns, "MACD_"),
        find_col(df.columns, "MACDs_"),
        find_col(df.columns, "BBU_"),
        find_col(df.columns, "RSI_"),
    )
    if not all([macd_col, macds_col, bbu_col, rsi_col]):
        return None
    if (
        prev[macd_col] <= prev[macds_col]
        and last[macd_col] > last[macds_col]
        and last["close"] > last[bbu_col]
        and last["close"] > last["VWAP_D"]
        and last[rsi_col] < 68
    ):
...
def analyze_breakout_squeeze_pro(df, params, rvol, adx_value):
    ...
    bbu_col, bbl_col, kcu_col, kcl_col = (
        find_col(df.columns, "BBU_"),
        find_col(df.columns, "BBL_"),
        find_col(df.columns, "KCUe_"),
        find_col(df.columns, "KCLEe_"),
    )
    ...
    if (
        is_in_squeeze
        and (last["close"] > last[bbu_col])
        and (last["volume"] > df["volume"].rolling(20).mean().iloc[-2] * 1.5)
        and (df["OBV"].iloc[-2] > df["OBV"].iloc[-3])
    ):
...
async def analyze_support_rebound(df, params, rvol, adx_value, exchange, symbol):
    try:
        ohlcv_1h = await safe_api_call(
            lambda: exchange.fetch_ohlcv(symbol, "1h", limit=100)
        )
        ...
        if not closest_support or (
            (current_price - closest_support) / closest_support * 100 > 1.0
        ):
            return None
        last_candle_15m = df.iloc[-2]
        if (
            last_candle_15m["close"] > last_candle_15m["open"]
            and last_candle_15m["volume"]
            > df["volume"].rolling(window=20).mean().iloc[-2] * 1.5
        ):
...
def analyze_sniper_pro(df, params, rvol, adx_value):
    try:
        ...
        highest_high, lowest_low = (
            compression_df["high"].max(),
            compression_df["low"].min(),
        )
        if lowest_low <= 0:
            return None
        ...
        if volatility < 12.0:
            ...
            if (
                last_candle["close"] > highest_high
                and last_candle["volume"] > compression_df["volume"].mean() * 2
            ):
...
def analyze_rsi_divergence(df, params, rvol, adx_value):
    ...
    price_troughs_idx, _ = find_peaks(
        -subset["low"], distance=params.get("peak_trough_lookback", 5)
    )
    rsi_troughs_idx, _ = find_peaks(
        -subset[rsi_col], distance=params.get("peak_trough_lookback", 5)
    )
    if len(price_troughs_idx) >= 2 and len(rsi_troughs_idx) >= 2:
        p_low1_idx, p_low2_idx = price_troughs_idx[-2], price_troughs_idx[-1]
        r_low1_idx, r_low2_idx = rsi_troughs_idx[-2], rsi_troughs_idx[-1]
        is_divergence = (
            subset.iloc[p_low2_idx]["low"] < subset.iloc[p_low1_idx]["low"]
            and subset.iloc[r_low2_idx][rsi_col] > subset.iloc[r_low1_idx][rsi_col]
        )
        if is_divergence:
            rsi_exits_oversold = (
                subset.iloc[r_low1_idx][rsi_col] < 35 and subset.iloc[-2][rsi_col] > 40
            )
            confirmation_price = subset.iloc[p_low2_idx:]["high"].max()
            price_confirmed = df.iloc[-2]["close"] > confirmation_price
            if (
                not params.get("confirm_with_rsi_exit", True) or rsi_exits_oversold
            ) and price_confirmed:
...
def analyze_supertrend_pullback(df, params, rvol, adx_value):
    df.ta.supertrend(
        length=params.get("atr_period", 10),
    ...
    if prev[st_dir_col] == -1 and last[st_dir_col] == 1:
        recent_swing_high = (
            df["high"].iloc[-params.get("swing_high_lookback", 10) : -2].max()
        )
...
async def get_okx_markets():
    ...
    valid_markets = [
        t
        for t in bot_data.all_markets
        if "USDT" in t["symbol"]
        and t.get("quoteVolume", 0)
        > settings["liquidity_filters"]["min_quote_volume_24h_usd"]
...
async def fetch_ohlcv_batch(exchange, symbols, timeframe, limit):
    tasks = [
        safe_api_call(lambda s=s: exchange.fetch_ohlcv(s, timeframe, limit=limit))
        for s in symbols
    ]
    results = await asyncio.gather(*tasks)
    return {
        symbols[i]: results[i] for i in range(len(symbols)) if results[i] is not None
    }
    ...
    while not queue.empty():
        try:
            ...
            if "whale_radar" in settings["active_scanners"]:
                whale_radar_signal = await analyze_whale_radar(
                    df.copy(), {}, 0, 0, exchange, symbol
                )
                if (
                    whale_radar_signal
                    and spread_percent
                    <= settings["spread_filter"]["max_spread_percent"] * 2
                ):
                    ...
                    stop_loss, take_profit = entry_price - risk, entry_price + (
                        risk * settings["risk_reward_ratio"]
                    )
                    signals_list.append(
                        {
                            "symbol": symbol,
            ...
            if settings.get("multi_timeframe_enabled", True):
                ohlcv_htf = await safe_api_call(
                    lambda: exchange.fetch_ohlcv(
                        symbol, settings.get("multi_timeframe_htf"), limit=220
                    )
                )
                if ohlcv_htf and len(ohlcv_htf) > 200:
                    ...
                    if ema_col_name_htf and pd.notna(df_htf[ema_col_name_htf].iloc[-2]):
                        is_htf_bullish = (
                            df_htf["close"].iloc[-2] > df_htf[ema_col_name_htf].iloc[-2]
                        )
            ...
            atr_period, min_atr_percent = vol_filters.get(
                "atr_period_for_filter", 14
            ), vol_filters.get("min_atr_percent", 0.8)
            ...
            atr_percent = (
                (df[atr_col_name].iloc[-2] / last_close) * 100 if last_close > 0 else 0
            )
            ...
            if settings.get("adx_filter_enabled", False):
                ...
                adx_value = (
                    df[adx_col].iloc[-2]
                    if adx_col and pd.notna(df[adx_col].iloc[-2])
                    else 0
                )
            ...
            for name in settings["active_scanners"]:
                ...
                result = (
                    await strategy_func(**func_args)
                    if asyncio.iscoroutinefunction(strategy_func)
                    else strategy_func(
                        **{
                            k: v
                            for k, v in func_args.items()
                            if k not in ["exchange", "symbol"]
                        }
                    )
                )
                if result:
            ...
            if confirmed_reasons:
                reason_str, strength = " + ".join(set(confirmed_reasons)), len(
                ...
                if settings.get("adaptive_intelligence_enabled", True):
                    ...
                    if perf:
                        if perf["win_rate"] < 50 and perf["total_trades"] > 5:
                            trade_weight = 1 - (
                                settings["dynamic_sizing_max_decrease_pct"] / 100.0
                            )
                        elif perf["win_rate"] > 70 and perf["profit_factor"] > 1.5:
                            trade_weight = 1 + (
                                settings["dynamic_sizing_max_increase_pct"] / 100.0
                            )

                        if (
                            perf["win_rate"]
                            < settings["strategy_deactivation_threshold_wr"]
                            and perf["total_trades"]
                            > settings["strategy_analysis_min_trades"]
                        ):
                            logger.warning(
                                f"Signal for {symbol} from weak strategy '{primary_reason}' ignored."
                            )
                            queue.task_done()
                            continue
...
async def handle_order_update(order_data):
    ...
    if order_data.get("state") == "filled" and order_data.get("side") == "buy":
        logger.info(
            f"Fast Reporter: Received fill for order {order_data['ordId']}. Activating trade..."
        )
...
async def activate_trade(order_id, symbol):
    """
    [النسخة النهائية المطورة V9.0]
    - تفعل الصفقة وتضيفها إلى الذاكرة المؤقتة (cache) للمعالجة الفورية.
    """
    bot = bot_data.application.bot
    try:
        order_details = await safe_api_call(
            lambda: bot_data.exchange.fetch_order(order_id, symbol)
        )
        if not order_details:
            logger.error(
                f"Could not fetch order details for activation of {order_id}. API call failed."
            )
            return
        ...
        logger.error(
            f"Could not fetch order details for activation of {order_id}: {e}",
            exc_info=True,
        )
        return
    ...
    async with aiosqlite.connect(DB_FILE) as conn:
        ...
        trade = await (
            await conn.execute(
                "SELECT * FROM trades WHERE order_id = ? AND status = 'pending'",
                (order_id,),
            )
        ).fetchone()
        ...
        if not trade:
            logger.info(
                f"Activation ignored for {order_id}: Trade not found or not pending."
            )
            return
        ...
        # نقوم بتحميل بيانات الصفقة الكاملة بعد تحديثها ونضعها في الذاكرة المؤقتة
        # لكي يتمكن الحارس (Guardian) من الوصول إليها فوراً.
        trade_full_details_cursor = await conn.execute(
            "SELECT * FROM trades WHERE id = ?", (trade["id"],)
        )
        trade_full_details = await trade_full_details_cursor.fetchone()
        if trade_full_details:
            bot_data.active_trades_cache[symbol] = dict(trade_full_details)
            logger.info(
                f"Trade #{trade['id']} loaded into active cache for real-time processing."
            )
        # --- [نهاية التعديل] ---
    ...
    usdt_remaining = (
        balance_after.get("USDT", {}).get("free", 0) if balance_after else 0
    )
    ...
    reasons_ar = " + ".join(
        [
            STRATEGY_NAMES_AR.get(r.strip(), r.strip())
            for r in trade["reason"].split(" + ")
        ]
    )
...
async def initiate_real_trade(signal, settings, exchange, bot):
    ...
    try:
        ...
        try:
            ...
            min_notional_str = market.get("limits", {}).get("notional", {}).get(
                "min"
            ...
            if min_notional_str is not None:
                ...
                if trade_size < required_size:
                    logger.warning(
                        f"Trade for {signal['symbol']} aborted. Trade size ({trade_size:.2f} USDT) is below the required minimum ({required_size:.2f} USDT)."
                    )
                    return False
        except Exception as e:
            logger.error(
                f"Could not fetch market rules for {signal['symbol']}: {e}. Skipping trade to be safe."
            )
            return False
        ...
        if usdt_balance < trade_size:
            logger.error(
                f"Insufficient USDT for {signal['symbol']}. Have: {usdt_balance:,.2f}, Need: {trade_size:,.2f}"
            )
            return False
        ...
        buy_order = await safe_api_call(
            lambda: exchange.create_market_buy_order(signal["symbol"], formatted_amount)
        )
        if not buy_order:
            return False
        ...
        if await log_pending_trade_to_db(signal, buy_order):
            ...
            reasons_ar = " + ".join(
                [
                    STRATEGY_NAMES_AR.get(r.strip(), r.strip())
                    for r in signal["reason"].split(" + ")
                ]
            )
            # نفترض وجود دالة build_enriched_message إذا كان الكود يدعمها
            if "build_enriched_message" in globals():
                msg_data = build_enriched_message(
                    "entry_alert", {**signal, "reason_ar": reasons_ar}
                )
            ...
            await safe_send_message(
                bot,
                f"⏳ **تم إرسال أمر الشراء لـ {signal['symbol']}. في انتظار تأكيد التنفيذ...**",
            )
            return True
        else:
            logger.critical(
                f"CRITICAL: Failed to log pending trade for {signal['symbol']}. Cancelling order {buy_order['id']}."
            )
            await safe_api_call(
                lambda: exchange.cancel_order(buy_order["id"], signal["symbol"])
            )
            return False
...
async def log_candidate_to_db(signal):
    try:
        async with aiosqlite.connect(DB_FILE) as conn:
            exists = await (
                await conn.execute(
                    "SELECT 1 FROM trade_candidates WHERE symbol = ? AND status = 'pending'",
                    (signal["symbol"],),
                )
            ).fetchone()
            if not exists:
                ...
                await conn.commit()
                logger.info(
                    f"New trade candidate logged for {signal['symbol']} for Wise Man review."
                )
    except Exception as e:
...
async def perform_scan(context: ContextTypes.DEFAULT_TYPE):
    async with scan_lock:
        if not bot_data.trading_enabled:
            logger.warning(
                "Scan skipped: Trading is disabled by circuit breaker or manually."
            )
            return
        ...
        try:
            ...
            if not balance:
                logger.error("Failed to fetch balance for scan check, skipping scan.")
                return
            usdt_balance = balance.get("USDT", {}).get("free", 0.0)
            trade_size_min_check = settings["real_trade_size_usdt"] * 0.98
            if usdt_balance < trade_size_min_check:
                logger.error(
                    f"Scan skipped: Insufficient USDT balance ({usdt_balance:,.2f} < {trade_size_min_check:,.2f}) to open a trade."
                )
                return
        except Exception as e:
            logger.error(f"Failed to fetch balance for scan check: {e}")
            return
        ...
        async with aiosqlite.connect(DB_FILE) as conn:
            active_trades_count = (
                await (
                    await conn.execute(
                        "SELECT COUNT(*) FROM trades WHERE status = 'active' OR status = 'pending'"
                    )
                ).fetchone()
            )[0]
        ...
        ohlcv_data = await fetch_ohlcv_batch(
            bot_data.exchange, symbols_to_scan, TIMEFRAME, 220
        )
        ...
        for market in top_markets:
            if market["symbol"] in ohlcv_data:
                await queue.put(
                    {"market": market, "ohlcv": ohlcv_data[market["symbol"]]}
                )

        worker_tasks = [
            asyncio.create_task(worker_batch(queue, signals_found, analysis_errors))
            for _ in range(settings.get("worker_threads", 10))
        ]
        await queue.join()
        ...
        if signals_found:
            logger.info(
                f"Scan found {len(signals_found)} new candidates. Logging them for the Wise Man to review."
            )
        ...
        bot_data.last_scan_info = {
            "start_time": datetime.fromtimestamp(scan_start_time, EGYPT_TZ).strftime(
                "%Y-%m-%d %H:%M:%S"
            ),
...
async def exponential_backoff_with_jitter(run_coro, *args, **kwargs):
    ...
    while True:
        try:
            ...
            logger.warning(
                f"Coroutine {run_coro.__name__} exited without error. Restarting after {base_delay}s..."
            )
            ...
            logger.error(
                f"Coroutine {run_coro.__name__} failed: {e}. Retrying in {total_delay:.2f} seconds..."
            )
...
async def handle_filled_buy_order(order_data):
    ...
    if float(order_data.get("avgPx", 0)) > 0:
        logger.info(
            f"Fast Reporter: Received fill for order {order_id}. Activating trade..."
        )
        ...
        async with websockets.connect(
            ...
            if login_response.get("code") == "0":
                ...
                raise ConnectionAbortedError(
                    f"Private WebSocket authentication failed: {login_response}"
                )
...
class PublicWebSocketManager:
    ...
    async def _send_op(self, op, symbols):
        ...
        try:
                ...
                json.dumps(
                    {
                        "op": op,
                        "args": [
                            {"channel": "tickers", "instId": s.replace("/", "-")}
                            for s in symbols
                        ],
                    }
                )
            ...
            logger.warning(
                f"Could not send '{op}' operation; public websocket is closed."
            )
...
# =======================================================================================
# --- 🛡️ TradeGuardian v3.1 (Final & Focused Build) 🛡️ ---
# =======================================================================================
# هذا هو الكود الكامل والنهائي للحارس، يحل مشكلة الوقف والخروج الديناميكي.


class TradeGuardian:
    def __init__(self, application, command_queue):
        self.application = application
        self.command_queue = command_queue
        self.tactical_overrides = {}
        self.tick_counters = defaultdict(int)
        self.atr_cache = {}
        logger.info("🛡️ TradeGuardian v3.1 (Focused & Hardened) Initialized.")

    async def command_listener(self):
        """[مهمة خلفية] يستمع بشكل دائم للأوامر القادمة من الرجل الحكيم (WiseMan)."""
        logger.info("Guardian's Command Listener is active.")
        while True:
            try:
                command = await self.command_queue.get()
                if command and command.get("task") == "UPDATE_TACTICS":
                    symbol, state, params = (
                        command["symbol"],
                        command["state"],
                        command.get("params", {}),
                    )
                    if symbol not in self.tactical_overrides:
                        self.tactical_overrides[symbol] = {}
                    self.tactical_overrides[symbol]["state"] = state
                    self.tactical_overrides[symbol].update(params)
                    logger.info(
                        f"Guardian received TACTICAL command for {symbol}: Set state to {state} with params {params}"
                    )
                self.command_queue.task_done()
            except Exception as e:
                logger.error(f"Command Listener crashed: {e}", exc_info=True)
                await asyncio.sleep(5)

    async def _get_atr(self, symbol: str) -> float | None:
        """يجلب قيمة ATR للعملة مع استخدام ذاكرة تخزين مؤقتة لتجنب طلبات API المفرطة."""
        now = time.time()
        if symbol in self.atr_cache and (
            now - self.atr_cache[symbol]["timestamp"] < 180
        ):  # Cache for 3 minutes
            return self.atr_cache[symbol]["value"]
        try:
            ohlcv = await safe_api_call(
                lambda: bot_data.exchange.fetch_ohlcv(symbol, "5m", limit=15)
            )
            ...
            if (
                ...
                and pd.notna(current_atr := atr_series.iloc[-1])
            ):
    ...
    async def handle_ticker_update(self, ticker_data):
        """[العقل التنفيذي] يعمل مع كل نبضة سعر لاتخاذ قرارات فورية."""
        async with trade_management_lock:
            symbol, current_price = ticker_data["instId"].replace("-", "/"), float(
                ticker_data["last"]
            )

            # --- استخدام الذاكرة المؤقتة للسرعة ---
            if (
                symbol not in bot_data.active_trades_cache
                or bot_data.active_trades_cache[symbol]["status"] != "active"
            ):
                return

            trade = bot_data.active_trades_cache[symbol]
            try:
                settings = bot_data.settings

                # --- 1. الأمان: "قاعدة الثلاث نبضات" لوقف الخسارة ---
                if current_price <= trade["stop_loss"]:
                    self.tick_counters[symbol] += 1
                    if self.tick_counters[symbol] >= 3:
                        logger.warning(
                            f"🛡️ SL CONFIRMED for #{trade['id']} after {self.tick_counters[symbol]} ticks. Executing closure."
                        )
                        reason = (
                            "فاشلة (وقف مؤكد)"
                            if trade["stop_loss"] <= trade["entry_price"]
                            else "ناجحة (وقف ربح مؤكد)"
                        )
                        await self._close_trade(trade, reason, current_price)
                        return
                elif self.tick_counters[symbol] > 0:
                    self.tick_counters[symbol] = 0  # السعر تعافى، إعادة تعيين العداد

                # --- تحديث أعلى سعر ---
                new_highest_price = max(trade.get("highest_price", 0), current_price)
                if new_highest_price > trade.get("highest_price", 0):
                    bot_data.active_trades_cache[symbol][
                        "highest_price"
                    ] = new_highest_price

                # --- 2. الربحية: "كاشف فقدان الزخم" (حلب العملة) ---
                activation_pct = settings["trailing_sl_activation_percent"]
                profit_pct = (
                ...
                if profit_pct >= activation_pct:
                    tactic_params = self.tactical_overrides.get(symbol, {})
                    callback_pct = tactic_params.get(
                        "callback_pct", settings["trailing_sl_callback_percent"]
                    )
                    exit_trigger_price = new_highest_price * (
                        1 - (callback_pct / 100.0)
                    )
                    if current_price < exit_trigger_price:
                        logger.warning(
                            f"🛡️ PEAK MOMENTUM LOST for #{trade['id']}. Closing trade."
                        )
                        await self._close_trade(
                            trade, "ناجحة (فقدان الزخم)", current_price
                        )
                        return

                # --- 3. منطق الدعم: رفع الوقف المتحرك العادي (باستخدام ATR) ---
                current_atr = await self._get_atr(symbol)
                if current_atr:
                    atr_multiplier = settings.get("atr_trailing_multiplier", 2.5)
                    new_sl_candidate = new_highest_price - (
                        current_atr * atr_multiplier
                    )
                    if (
                        new_sl_candidate > trade["stop_loss"]
                        and new_sl_candidate > trade["entry_price"]
                    ):
                        bot_data.active_trades_cache[symbol][
                            "stop_loss"
                        ] = new_sl_candidate
                        logger.info(
                            f"Guardian raised ATR Trailing Stop for #{trade['id']} to {new_sl_candidate:.4f}"
                        )
            except Exception as e:
                logger.error(
                    f"CRITICAL Guardian Ticker Error for {symbol}: {e}", exc_info=True
                )

    async def _close_trade(self, trade: dict, reason: str, close_price: float):
        """[النسخة المُحصّنة] بروتوكول الإغلاق الكامل المأخوذ من بوت Binance والمُكيّف لبوت OKX."""
        symbol, trade_id = trade["symbol"], trade["id"]
        bot = self.application.bot
        if symbol in self.tick_counters:
            del self.tick_counters[symbol]
        if symbol in self.tactical_overrides:
            del self.tactical_overrides[symbol]
        if symbol in bot_data.active_trades_cache:
            del bot_data.active_trades_cache[symbol]

        logger.info(
            f"🛡️ Initiating Hardened Closure for trade #{trade_id} [{symbol}]. Reason: {reason}"
        )
        try:
            base_currency = symbol.split("/")[0]
            balance = await safe_api_call(lambda: bot_data.exchange.fetch_balance())
            available_quantity = (
                balance.get(base_currency, {}).get("free", 0.0) if balance else 0.0
            )
            if available_quantity > 0.00000001:
                market = bot_data.exchange.market(symbol)
                quantity_to_sell = float(
                    bot_data.exchange.amount_to_precision(symbol, available_quantity)
                )
                min_qty_str = market.get("limits", {}).get("amount", {}).get("min")
                min_qty = float(min_qty_str) if min_qty_str else 0
                if not (min_qty > 0 and quantity_to_sell < min_qty):
                    await safe_api_call(
                        lambda: bot_data.exchange.create_market_sell_order(
            ...
            pnl_percent = (
                (close_price / trade["entry_price"] - 1) * 100
                if trade["entry_price"] > 0
                else 0
            )
            ...
            duration, highest_price = datetime.now(EGYPT_TZ).astimezone(
                start_dt.tzinfo
            ) - start_dt, trade.get("highest_price", 0)
            exit_efficiency = 0.0
            if highest_price > trade["entry_price"]:
                potential_pnl = (highest_price - trade["entry_price"]) * trade[
                    "quantity"
                ]
                if potential_pnl > 0:
                    exit_efficiency = max(0, min((pnl / potential_pnl) * 100, 100))

            emoji, reasons_ar = "✅" if pnl >= 0 else "🛑", " + ".join(
                [
                    STRATEGY_NAMES_AR.get(r.strip(), r.strip())
                    for r in trade["reason"].split(" + ")
                ]
            ...
            msg = (
                ...
                f"▫️ **العملة:** `{symbol}` | **سبب الإغلاق:** `{reason}`\n"
                f"▫️ **الاستراتيجية:** `{reasons_ar}`\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"💰 **صافي الربح/الخسارة:** `${pnl:,.2f}` `({pnl_percent:+.2f}%)`\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"⏳ **مدة الصفقة:** `{str(duration).split('.')[0]}`\n"
                f"📉 **سعر الدخول:** `${trade['entry_price']:,.4f}` | 📈 **سعر الخروج:** `${close_price:,.4f}`\n"
                f"🔝 **أعلى سعر:** `${highest_price:,.4f}` | 🧠 **كفاءة الخروج:** `{exit_efficiency:.2f}%`"
            )
            await safe_send_message(bot, msg)
        except Exception as e:
            logger.critical(
                f"CRITICAL: Hardened closure for #{trade_id} failed: {e}", exc_info=True
            )
            async with aiosqlite.connect(DB_FILE) as conn:
                await conn.execute(
                    "UPDATE trades SET status = 'closure_failed' WHERE id = ?",
                    (trade_id,),
                )
                await conn.commit()
        finally:
...
async def the_supervisor_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    async with aiosqlite.connect(DB_FILE) as conn:
        ...
        stuck_trades = await (
            await conn.execute(
                "SELECT * FROM trades WHERE status = 'pending' AND timestamp <= ?",
                (two_mins_ago,),
            )
        ).fetchall()
        for trade_data in stuck_trades:
            ...
            logger.warning(
                f"🕵️ Supervisor: Found abandoned trade #{trade['id']}. Investigating.",
                extra={"trade_id": trade["id"]},
            )
            try:
                order_status = await safe_api_call(
                    lambda: bot_data.exchange.fetch_order(order_id, symbol)
                )
                if not order_status:
                    continue
                if (
                    ...
                    and order_status.get("filled", 0) > 0
                ):
                    await activate_trade(order_id, symbol)
                elif order_status["status"] in ["canceled", "expired"]:
                    await conn.execute(
                        "DELETE FROM trades WHERE id = ?", (trade["id"],)
                    )
                await conn.commit()
                ...
                logger.error(
                    f"🕵️ Supervisor error processing stuck trade #{trade['id']}: {e}",
                    extra={"trade_id": trade["id"]},
                )

        failed_trades = await (
            await conn.execute(
                "SELECT * FROM trades WHERE status = 'closure_failed' OR status = 'incubated'"
            )
        ).fetchall()
        for trade_data in failed_trades:
            trade = dict(trade_data)
            logger.warning(
                f"🚨 Supervisor: Found failed closure for trade #{trade['id']}. Retrying intervention."
            )
            try:
                ticker = await safe_api_call(
                    lambda: bot_data.exchange.fetch_ticker(trade["symbol"])
                )
                if ticker:
                    current_price = ticker.get("last")
                    if current_price:
                        await TradeGuardian(context.application)._close_trade(
                            trade, "إغلاق إجباري (مشرف)", current_price
                        )
            except Exception as e:
                logger.error(
                    f"🚨 Supervisor failed to intervene for trade #{trade['id']}: {e}"
                )
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "أهلاً بك في **بوت OKX V8.1 (النسخة النهائية)**",
...
async def manual_scan_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not bot_data.trading_enabled:
        await (update.message or update.callback_query.message).reply_text(
            "🔬 الفحص محظور. مفتاح الإيقاف مفعل."
        )
        return
    await (update.message or update.callback_query.message).reply_text(
        "🔬 أمر فحص يدوي... قد يستغرق بعض الوقت."
    )
...
async def show_dashboard_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    ks_status_text = (
        "مفتاح الإيقاف (مفعل)" if not bot_data.trading_enabled else "الحالة (طبيعية)"
    )
    keyboard = [
        [
            InlineKeyboardButton(
                "💼 نظرة عامة على المحفظة", callback_data="db_portfolio"
            ),
            InlineKeyboardButton("📈 الصفقات النشطة", callback_data="db_trades"),
        ],
        [
            InlineKeyboardButton("📜 سجل الصفقات المغلقة", callback_data="db_history"),
            InlineKeyboardButton("📊 الإحصائيات والأداء", callback_data="db_stats"),
        ],
        [
        ...
        [
            InlineKeyboardButton(
                f"{ks_status_emoji} {ks_status_text}",
...
async def show_settings_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [
        [
            InlineKeyboardButton(
                "🧠 إعدادات الذكاء التكيفي", callback_data="settings_adaptive"
            )
        ],
        [
            InlineKeyboardButton(
                "🎛️ تعديل المعايير المتقدمة", callback_data="settings_params"
            )
        ],
        [
            InlineKeyboardButton(
                "🔭 تفعيل/تعطيل الماسحات", callback_data="settings_scanners"
            )
        ],
        [InlineKeyboardButton("🗂️ أنماط جاهزة", callback_data="settings_presets")],
        [
            InlineKeyboardButton(
                "🚫 القائمة السوداء", callback_data="settings_blacklist"
            ),
...
async def universal_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if (
        "setting_to_change" in context.user_data
        or "blacklist_action" in context.user_data
    ):
...
async def show_diagnostics_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    crypto_status = (
        "متاحة ✅" if CRYPTO_AVAILABLE else "غير متاحة ❌"
    )  # --- [تعديل V8.1]
    ...
    scanners_list = "\n".join(
        [
            f"  - {STRATEGY_NAMES_AR.get(key, key)}"
            for key in s.get("active_scanners", [])
        ]
    )
    scan_job = context.job_queue.get_jobs_by_name("perform_scan")
    next_scan_time = (
        scan_job[0].next_t.astimezone(EGYPT_TZ).strftime("%H:%M:%S")
        if scan_job and scan_job[0].next_t
        else "N/A"
    )
    db_size = (
        f"{os.path.getsize(DB_FILE) / 1024:.2f} KB"
    ...
    try:
        async with aiosqlite.connect(DB_FILE) as conn:
            total_trades = (
                await (await conn.execute("SELECT COUNT(*) FROM trades")).fetchone()
            )[0]
            active_trades = (
                await (
                    await conn.execute(
    ...
    await safe_edit_message(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [InlineKeyboardButton("🔄 تحديث", callback_data="db_diagnostics")],
                [
                    InlineKeyboardButton(
...
async def send_daily_report(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        async with aiosqlite.connect(DB_FILE) as conn:
            ...
            closed_today = await (
                await conn.execute(
                    "SELECT * FROM trades WHERE status LIKE '%(%' AND date(timestamp) = ?",
                    (today_str,),
                )
            ).fetchall()
        ...
        wins = [
            ...
            if ("ناجحة" in t["status"] or "تأمين" in t["status"])
            and t["pnl_usdt"] is not None
        ]
        losses = [
            t
            for t in closed_today
            if "فاشلة" in t["status"] and t["pnl_usdt"] is not None
        ]
        ...
        drawdown = (
            (abs(worst_trade_pnl) / total_pnl * 100)
            if total_pnl > 0 and worst_trade_pnl < 0
            else 0
        )

        best_trade = max(
            closed_today, key=lambda t: t.get("pnl_usdt", -float("inf")), default=None
        )
        worst_trade = min(
            closed_today, key=lambda t: t.get("pnl_usdt", float("inf")), default=None
        )
        ...
        strategy_counter = Counter(
            r for t in closed_today for r in t["reason"].split(" + ")
        )
        most_active_strategy_en = (
            strategy_counter.most_common(1)[0][0] if strategy_counter else "N/A"
        )
        most_active_strategy_ar = STRATEGY_NAMES_AR.get(
            most_active_strategy_en.split(" ")[0], most_active_strategy_en
        )
        ...
        report_data = {
            ...
            "best_trade": (
                {"symbol": best_trade["symbol"], "pnl": best_trade["pnl_usdt"]}
                if best_trade
                else {"symbol": "N/A", "pnl": 0}
            ),
            "worst_trade": (
                {"symbol": worst_trade["symbol"], "pnl": worst_trade["pnl_usdt"]}
                if worst_trade
                else {"symbol": "N/A", "pnl": 0}
            ),
        ...
        chart_config = {
            ...
            "options": {
                "title": {"display": True, "text": f"ملخص أداء يوم {today_str}"}
            },
        }
        chart_url = f"https://quickchart.io/chart?c={json.dumps(chart_config)}"

        msg_content = build_enriched_message(
            "daily_summary", {"report_data": report_data, "chart_url": chart_url}
        )
        await safe_send_message(
            context.bot, msg_content["text"], **msg_content["kwargs"]
        )
...
async def show_trades_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    async with aiosqlite.connect(DB_FILE) as conn:
        ...
        trades = await (
            await conn.execute(
                "SELECT id, symbol, status FROM trades WHERE status = 'active' OR status = 'pending' ORDER BY id DESC"
            )
        ).fetchall()
    if not trades:
...
async def check_trade_details(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not trade:
        await query.answer("لم يتم العثور على الصفقة.")
        return
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "🚨 بيع فوري (بسعر السوق)",
                callback_data=f"manual_sell_confirm_{trade_id}",
            )
        ],
    ...
    if trade["status"] == "pending":
        ...
        try:
            ...
            pnl_text = (
                f"💰 **الربح/الخسارة الحالية:** `${pnl:+.2f}` ({pnl_percent:+.2f}%)"
            )
...
async def show_mood_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    translated_headlines, translation_success = await translate_text_gemini(
        original_headlines
    )
    ...
    if all_markets:
        sorted_by_change = sorted(
            [m for m in all_markets if m.get("percentage") is not None],
    ...
    if fng_index and fng_index > 65:
        verdict = (
            "المؤشرات الفنية إيجابية ولكن مع وجود طمع في السوق، يرجى الحذر من التقلبات."
        )
    ...
    gainers_str = (
        "\n".join(
            [
                f"  `{g['symbol']}` `({g.get('percentage', 0):+.2f}%)`"
    ...
    losers_str = (
        "\n".join(
            [
                f"  `{l['symbol']}` `({l.get('percentage', 0):+.2f}%)`"
    ...
    news_header = (
        "📰 آخر الأخبار (مترجمة آلياً):"
        if translation_success
        else "📰 آخر الأخبار (الترجمة غير متاحة):"
    )
    news_str = (
    ...
    keyboard = [
        [InlineKeyboardButton("🔄 تحديث", callback_data="db_mood")],
        [
            InlineKeyboardButton(
                "🔙 العودة للوحة التحكم", callback_data="back_to_dashboard"
            )
...
async def show_strategy_report_command(
    update: Update, context: ContextTypes.DEFAULT_TYPE
):
    if not bot_data.strategy_performance:
        await safe_edit_message(
            update.callback_query,
            "لا توجد بيانات أداء حاليًا. يرجى الانتظار بعد إغلاق بعض الصفقات.",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "🔙 العودة للإحصائيات", callback_data="db_stats"
                        )
                    ]
                ]
            ),
    ...
    sorted_strategies = sorted(
        bot_data.strategy_performance.items(),
        key=lambda item: item[1]["total_trades"],
        reverse=True,
    )
    ...
    await safe_edit_message(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "📊 عرض الإحصائيات العامة", callback_data="db_stats"
                    )
                ],
                [
                    InlineKeyboardButton(
...
async def show_stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    async with aiosqlite.connect(DB_FILE) as conn:
        conn.row_factory = aiosqlite.Row
        cursor = await conn.execute(
            "SELECT pnl_usdt, status FROM trades WHERE status LIKE '%(%'"
        )
        trades_data = await cursor.fetchall()
    if not trades_data:
        await safe_edit_message(
            update.callback_query,
            "لم يتم إغلاق أي صفقات بعد.",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
    ...
    profit_factor = (
        sum(wins_data) / abs(sum(losses_data))
        if sum(losses_data) != 0
        else float("inf")
    )
    message = (
    ...
    await safe_edit_message(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        "📜 عرض تقرير الاستراتيجيات", callback_data="db_strategy_report"
                    )
                ],
                [
                    InlineKeyboardButton(
...
async def show_portfolio_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not balance:
            await safe_edit_message(
                query,
                "حدث خطأ أثناء جلب رصيد المحفظة.",
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "🔙 العودة", callback_data="back_to_dashboard"
                            )
                        ]
                    ]
                ),
        ...
        if assets_to_fetch:
            try:
                tickers = await safe_api_call(
                    lambda: bot_data.exchange.fetch_tickers(assets_to_fetch)
                )
            except Exception as e:
        ...
        for asset, total in owned_assets.items():
            symbol = f"{asset}/USDT"
            value_usdt = 0
            if tickers and symbol in tickers and tickers[symbol] is not None:
        ...
        async with aiosqlite.connect(DB_FILE) as conn:
            cursor_pnl = await conn.execute(
                "SELECT SUM(pnl_usdt) FROM trades WHERE status LIKE '%(%'"
            )
            total_realized_pnl = (await cursor_pnl.fetchone())[0] or 0.0
            cursor_trades = await conn.execute(
                "SELECT COUNT(*) FROM trades WHERE status = 'active'"
            )
        ...
        assets_str = (
            "\n".join(asset_details)
            if asset_details
            else "  لا توجد أصول أخرى بقيمة تزيد عن 1 دولار."
        )
        message = (
        ...
        keyboard = [
            [InlineKeyboardButton("🔄 تحديث", callback_data="db_portfolio")],
            [
                InlineKeyboardButton(
                    "🔙 العودة للوحة التحكم", callback_data="back_to_dashboard"
                )
        ...
        await safe_edit_message(
            query,
            f"حدث خطأ أثناء جلب رصيد المحفظة: {e}",
            reply_markup=InlineKeyboardMarkup(
...
async def show_trade_history_command(
    update: Update, context: ContextTypes.DEFAULT_TYPE
):
    async with aiosqlite.connect(DB_FILE) as conn:
        conn.row_factory = aiosqlite.Row
        cursor = await conn.execute(
            "SELECT symbol, pnl_usdt, status FROM trades WHERE status LIKE '%(%' ORDER BY id DESC LIMIT 10"
        )
    ...
    for trade in closed_trades:
        emoji = (
            "✅" if "ناجحة" in trade["status"] or "تأمين" in trade["status"] else "🛑"
        )
        pnl = trade["pnl_usdt"] or 0.0
        history_list.append(
            f"{emoji} `{trade['symbol']}` | الربح/الخسارة: `${pnl:,.2f}`"
        )
    text = "\n".join(history_list)
    keyboard = [
        [
...
async def show_adaptive_intelligence_menu(
    update: Update, context: ContextTypes.DEFAULT_TYPE
):
    s = bot_data.settings
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                bool_format("adaptive_intelligence_enabled", "تفعيل الذكاء التكيفي"),
                callback_data="param_toggle_adaptive_intelligence_enabled",
            )
        ],
        [
            InlineKeyboardButton(
                bool_format("wise_man_auto_close", "الإغلاق الآلي للرجل الحكيم"),
                callback_data="param_toggle_wise_man_auto_close",
            )
        ],
        [
            InlineKeyboardButton(
                bool_format("dynamic_trade_sizing_enabled", "الحجم الديناميكي للصفقات"),
                callback_data="param_toggle_dynamic_trade_sizing_enabled",
            )
        ],
        [
            InlineKeyboardButton(
                bool_format("strategy_proposal_enabled", "اقتراحات الاستراتيجيات"),
                callback_data="param_toggle_strategy_proposal_enabled",
            )
        ],
        [InlineKeyboardButton("--- معايير الضبط ---", callback_data="noop")],
        [
            InlineKeyboardButton(
                f"حد أدنى للتعطيل (WR%): {s.get('strategy_deactivation_threshold_wr', 45.0)}",
                callback_data="param_set_strategy_deactivation_threshold_wr",
            )
        ],
        [
            InlineKeyboardButton(
                f"أقل عدد صفقات للتحليل: {s.get('strategy_analysis_min_trades', 10)}",
                callback_data="param_set_strategy_analysis_min_trades",
            )
        ],
        [
            InlineKeyboardButton(
                f"أقصى زيادة للحجم (%): {s.get('dynamic_sizing_max_increase_pct', 25.0)}",
                callback_data="param_set_dynamic_sizing_max_increase_pct",
            )
        ],
        [
            InlineKeyboardButton(
                f"أقصى تخفيض للحجم (%): {s.get('dynamic_sizing_max_decrease_pct', 50.0)}",
                callback_data="param_set_dynamic_sizing_max_decrease_pct",
            )
        ],
    ...
    await safe_edit_message(
        update.callback_query,
        "🧠 **إعدادات الذكاء التكيفي**\n\nتحكم في كيفية تعلم البوت وتكيفه:",
        reply_markup=InlineKeyboardMarkup(keyboard),
    )
...
async def show_parameters_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton(
                ...
                callback_data="param_set_top_n_symbols_by_volume",
            ),
            InlineKeyboardButton(
                f"أقصى عدد للصفقات: {s['max_concurrent_trades']}",
                callback_data="param_set_max_concurrent_trades",
            ),
        ],
        [
            InlineKeyboardButton(
                f"عمال الفحص المتزامنين: {s['worker_threads']}",
                callback_data="param_set_worker_threads",
            )
        ],
        ...
        [
            InlineKeyboardButton(
                ...
                callback_data="param_set_real_trade_size_usdt",
            ),
            InlineKeyboardButton(
                f"مضاعف وقف الخسارة (ATR): {s['atr_sl_multiplier']}",
                callback_data="param_set_atr_sl_multiplier",
            ),
        ],
        [
            InlineKeyboardButton(
                f"نسبة المخاطرة/العائد: {s['risk_reward_ratio']}",
                callback_data="param_set_risk_reward_ratio",
            )
        ],
        [
            InlineKeyboardButton(
                f"أقل احتمالية نجاح: {s.get('min_win_probability', 0.6)*100:.0f}%",
                callback_data="param_set_min_win_probability",
            )
        ],
        [
            InlineKeyboardButton(
                bool_format("trailing_sl_enabled", "تفعيل الوقف المتحرك"),
                callback_data="param_toggle_trailing_sl_enabled",
            )
        ],
        [
            InlineKeyboardButton(
                f"تفعيل الوقف المتحرك (%): {s['trailing_sl_activation_percent']}",
                callback_data="param_set_trailing_sl_activation_percent",
            ),
            InlineKeyboardButton(
                f"مسافة الوقف المتحرك (%): {s['trailing_sl_callback_percent']}",
                callback_data="param_set_trailing_sl_callback_percent",
            ),
        ],
        [
            InlineKeyboardButton(
                f"عدد محاولات الإغلاق: {s['close_retries']}",
                callback_data="param_set_close_retries",
            )
        ],
        [
            InlineKeyboardButton(
                "--- إعدادات الإشعارات والفلترة ---", callback_data="noop"
            )
        ],
        [
            InlineKeyboardButton(
                bool_format(
                    "incremental_notifications_enabled", "إشعارات الربح المتزايدة"
                ),
                callback_data="param_toggle_incremental_notifications_enabled",
            )
        ],
        [
            InlineKeyboardButton(
                f"نسبة إشعار الربح (%): {s['incremental_notification_percent']}",
                callback_data="param_set_incremental_notification_percent",
            )
        ],
        [
            InlineKeyboardButton(
                f"مضاعف فلتر الحجم: {s['volume_filter_multiplier']}",
                callback_data="param_set_volume_filter_multiplier",
            )
        ],
        [
            InlineKeyboardButton(
                bool_format("multi_timeframe_enabled", "فلتر الأطر الزمنية"),
                callback_data="param_toggle_multi_timeframe_enabled",
            )
        ],
        [
            InlineKeyboardButton(
                bool_format("btc_trend_filter_enabled", "فلتر اتجاه BTC"),
                callback_data="param_toggle_btc_trend_filter_enabled",
            )
        ],
        [
            InlineKeyboardButton(
                f"فترة EMA للاتجاه: {get_nested_value(s, ['trend_filters', 'ema_period'])}",
                callback_data="param_set_trend_filters_ema_period",
            )
        ],
        [
            InlineKeyboardButton(
                f"أقصى سبريد مسموح (%): {get_nested_value(s, ['spread_filter', 'max_spread_percent'])}",
                callback_data="param_set_spread_filter_max_spread_percent",
            )
        ],
        [
            InlineKeyboardButton(
                f"أدنى ATR مسموح (%): {get_nested_value(s, ['volatility_filters', 'min_atr_percent'])}",
                callback_data="param_set_volatility_filters_min_atr_percent",
            )
        ],
        [
            InlineKeyboardButton(
                bool_format("market_mood_filter_enabled", "فلتر الخوف والطمع"),
                callback_data="param_toggle_market_mood_filter_enabled",
            ),
            InlineKeyboardButton(
                f"حد مؤشر الخوف: {s['fear_and_greed_threshold']}",
                callback_data="param_set_fear_and_greed_threshold",
            ),
        ],
        [
            InlineKeyboardButton(
                bool_format("adx_filter_enabled", "فلتر ADX"),
                callback_data="param_toggle_adx_filter_enabled",
            ),
            InlineKeyboardButton(
                f"مستوى فلتر ADX: {s['adx_filter_level']}",
                callback_data="param_set_adx_filter_level",
            ),
        ],
        [
            InlineKeyboardButton(
                bool_format("news_filter_enabled", "فلتر الأخبار والبيانات"),
                callback_data="param_toggle_news_filter_enabled",
            )
        ],
        [
            InlineKeyboardButton(
                "--- إعدادات الرجل الحكيم (حساسية الزخم) ---", callback_data="noop"
            )
        ],
        [
            InlineKeyboardButton(
                f"نسبة الربح للزخم القوي (%): {s.get('wise_man_strong_profit_pct', 3.0)}",
                callback_data="param_set_wise_man_strong_profit_pct",
            )
        ],
        [
            InlineKeyboardButton(
                f"مستوى ADX للزخم القوي: {s.get('wise_man_strong_adx_level', 30)}",
                callback_data="param_set_wise_man_strong_adx_level",
            )
        ],
        [InlineKeyboardButton("🔙 العودة للإعدادات", callback_data="settings_main")],
    ]
    await safe_edit_message(
        update.callback_query,
        "🎛️ **تعديل المعايير المتقدمة**\n\nاضغط على أي معيار لتعديل قيمته مباشرة:",
        reply_markup=InlineKeyboardMarkup(keyboard),
    )
...
async def show_scanners_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for key, name in STRATEGY_NAMES_AR.items():
        ...
        keyboard.append(
            [
                InlineKeyboardButton(
                    f"{status_emoji} {name}{perf_hint}",
                    callback_data=f"scanner_toggle_{key}",
                )
            ]
        )
    keyboard.append(
    ...
    await safe_edit_message(
        update.callback_query,
        "اختر الماسحات لتفعيلها أو تعطيلها (مع تلميح الأداء):",
        reply_markup=InlineKeyboardMarkup(keyboard),
    )
...
async def show_presets_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [
        ...
        [
            InlineKeyboardButton(
                "⚠️ فائق التساهل", callback_data="preset_set_very_lenient"
            )
        ],
...
async def show_blacklist_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [
            InlineKeyboardButton("➕ إضافة عملة", callback_data="blacklist_add"),
            InlineKeyboardButton("➖ إزالة عملة", callback_data="blacklist_remove"),
        ],
        [InlineKeyboardButton("🔙 العودة للإعدادات", callback_data="settings_main")],
    ]
    await safe_edit_message(
...
async def show_data_management_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keyboard = [
        [
            InlineKeyboardButton(
                "‼️ مسح كل الصفقات ‼️", callback_data="data_clear_confirm"
            )
        ],
        [InlineKeyboardButton("🔙 العودة للإعدادات", callback_data="settings_main")],
    ]
    await safe_edit_message(
        update.callback_query,
        "🗑️ *إدارة البيانات*\n\n**تحذير:** هذا الإجراء سيحذف سجل جميع الصفقات بشكل نهائي.",
        reply_markup=InlineKeyboardMarkup(keyboard),
    )


async def handle_clear_data_confirmation(
    update: Update, context: ContextTypes.DEFAULT_TYPE
):
    keyboard = [
        [
            InlineKeyboardButton(
                "نعم، متأكد. احذف كل شيء.", callback_data="data_clear_execute"
            )
        ],
    ...
    await safe_edit_message(
        update.callback_query,
        "🛑 **تأكيد نهائي: حذف البيانات**\n\nهل أنت متأكد أنك تريد حذف جميع بيانات الصفقات بشكل نهائي؟",
        reply_markup=InlineKeyboardMarkup(keyboard),
    )
...
async def handle_scanner_toggle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if scanner_key not in STRATEGY_NAMES_AR:
        logger.error(f"Invalid scanner key: '{scanner_key}'")
    ...
    determine_active_preset()
    await query.answer(
        f"{STRATEGY_NAMES_AR[scanner_key]} {'تم تفعيله' if scanner_key in active_scanners else 'تم تعطيله'}"
    )
...
async def handle_preset_set(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if preset_settings := SETTINGS_PRESETS.get(preset_key):
        ...
        adaptive_settings = {
            k: v for k, v in bot_data.settings.items() if k not in preset_settings
        }
...
async def handle_strategy_adjustment(
    update: Update, context: ContextTypes.DEFAULT_TYPE
):
    query = update.callback_query
    ...
    if not proposal or proposal.get("key") != proposal_key:
        await safe_edit_message(query, "انتهت صلاحية هذا الاقتراح.", reply_markup=None)
        return
    ...
    if action == "approve":
        ...
        if scanner_to_disable in bot_data.settings["active_scanners"]:
            ...
            await safe_edit_message(
                query,
                f"✅ **تمت الموافقة.** تم تعطيل استراتيجية '{STRATEGY_NAMES_AR.get(scanner_to_disable, scanner_to_disable)}'.",
                reply_markup=None,
            )
    else:
        await safe_edit_message(
            query, "❌ **تم الرفض.** لن يتم إجراء أي تغييرات.", reply_markup=None
        )

    bot_data.pending_strategy_proposal = {}


async def handle_parameter_selection(
    update: Update, context: ContextTypes.DEFAULT_TYPE
):
    query = update.callback_query
    ...
    if "_" in param_key:
        await query.message.reply_text(
            f"أرسل القيمة الرقمية الجديدة لـ `{param_key}`:\n\n*ملاحظة: هذا إعداد متقدم (متشعب)، سيتم تحديثه مباشرة.*",
            parse_mode=ParseMode.MARKDOWN,
        )
    else:
        await query.message.reply_text(
...
async def handle_toggle_parameter(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    determine_active_preset()
    if (
        "adaptive" in param_key
        or "strategy" in param_key
        or "dynamic" in param_key
        or "wise_man" in param_key
    ):
...
async def handle_blacklist_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.reply_text(
        f"أرسل رمز العملة التي تريد **{ 'إضافتها' if action == 'add' else 'إزالتها'}** (مثال: `BTC` أو `DOGE`)"
    )
...
async def handle_setting_value(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "blacklist_action" in context.user_data:
        ...
        if action == "add":
            if symbol not in blacklist:
                ...
                await update.message.reply_text(
                    f"✅ تم إضافة `{symbol}` إلى القائمة السوداء."
                )
            else:
            ...
            if symbol in blacklist:
                ...
                await update.message.reply_text(
                    f"✅ تم إزالة `{symbol}` من القائمة السوداء."
                )
            else:
                await update.message.reply_text(
                    f"⚠️ العملة `{symbol}` غير موجودة في القائمة."
                )
        ...
        await show_blacklist_menu(
            Update(update.update_id, callback_query=fake_query), context
        )
        return
    ...
    try:
        if setting_key in bot_data.settings and not isinstance(
            bot_data.settings[setting_key], dict
        ):
        ...
        determine_active_preset()
        await update.message.reply_text(
            f"✅ تم تحديث `{setting_key}` إلى `{new_value}`."
        )
        ...
        parent_menu_data = (
            "settings_adaptive"
            if any(k in setting_key for k in ["adaptive", "strategy", "dynamic"])
            else "settings_params"
        )
        fake_query = type(
            "Query",
            (),
            {
                "message": update.message,
                "data": parent_menu_data,
                "edit_message_text": (lambda *args, **kwargs: asyncio.sleep(0)),
                "answer": (lambda *args, **kwargs: asyncio.sleep(0)),
            },
        )
        if parent_menu_data == "settings_adaptive":
            await show_adaptive_intelligence_menu(
                Update(update.update_id, callback_query=fake_query), context
            )
        else:
            await show_parameters_menu(
                Update(update.update_id, callback_query=fake_query), context
            )


async def handle_manual_sell_confirmation(
    update: Update, context: ContextTypes.DEFAULT_TYPE
):
    query = update.callback_query
    trade_id = int(query.data.split("_")[-1])
    ...
    if not trade_data:
        await query.answer("لم يتم العثور على الصفقة.", show_alert=True)
        return
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "✅ نعم، قم بالبيع الآن",
                callback_data=f"manual_sell_execute_{trade_id}",
            )
        ],
...
async def handle_manual_sell_execute(
    update: Update, context: ContextTypes.DEFAULT_TYPE
):
    query = update.callback_query
    trade_id = int(query.data.split("_")[-1])
    ...
    try:
        async with aiosqlite.connect(DB_FILE) as conn:
            ...
            trade = await (
                await conn.execute(
                    "SELECT * FROM trades WHERE id = ? AND status = 'active'",
                    (trade_id,),
                )
            ).fetchone()
        ...
        if not trade:
            await query.answer(
                "لم يتم العثور على الصفقة أو أنها ليست نشطة.", show_alert=True
            )
        ...
        if not ticker:
            await safe_send_message(
                context.bot,
                f"🚨 فشل البيع اليدوي للصفقة #{trade_id}. السبب: تعذر جلب السعر الحالي.",
            )
        ...
        logger.error(
            f"Manual sell execution failed for trade #{trade_id}: {e}", exc_info=True
        )
        await safe_send_message(
            context.bot, f"🚨 فشل البيع اليدوي للصفقة #{trade_id}. السبب: {e}"
        )
        await query.answer("🚨 فشل أمر البيع. راجع السجلات.", show_alert=True)
...
async def button_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    route_map = {
        ...
        "db_daily_report": daily_report_command,
    ...
    try:
        ...
        logger.error(
            f"Error in button callback handler for data '{data}': {e}", exc_info=True
        )


async def post_init(application: Application):
    """
    [النسخة النهائية والمعدلة v9.1]
    - تقوم بتهيئة جميع المكونات الجديدة (الحارس، طابور الأوامر، الذاكرة المؤقتة).
    - تربط جميع الوحدات معًا بشكل صحيح عند بدء التشغيل.
    """
    global bot_data
    bot_data = BotState()

    logger.info("Performing post-initialization for OKX Bot v9.1 (Focused Build)...")

    # --- 1. فحص متغيرات البيئة وتهيئة قاعدة البيانات (يبقى كما هو) ---
    required_vars = [
        "TELEGRAM_BOT_TOKEN",
        "TELEGRAM_CHAT_ID",
        "OKX_API_KEY",
    ...
    try:
        ...
        raise RuntimeError("Failed to connect to OKX.")

    bot_data.application = application
    load_settings()

    # --- 3. [التعديل الحاسم] إنشاء وربط جميع الوحدات ---
    global wise_man, smart_brain

    # إنشاء الحارس (المنفذ السريع) وتمرير طابور الأوامر له
    bot_data.trade_guardian = TradeGuardian(application, command_queue)

    # إنشاء الرجل الحكيم (العقل المفكر) وتمرير الطابور له
    wise_man = WiseMan(
        exchange=bot_data.exchange,
        application=application,
        bot_data_ref=bot_data,
        db_file=DB_FILE,
        command_queue=command_queue,  # تمرير الطابور
    )

    smart_brain = EvolutionaryEngine(exchange=bot_data.exchange, db_file=DB_FILE)

    # --- 4. تحميل الصفقات النشطة إلى الذاكرة المؤقتة ---
    await load_active_trades_to_cache()

    # --- 5. تشغيل أنظمة الوقت الفعلي (WebSockets ومستمع الأوامر) ---
    bot_data.public_ws = PublicWebSocketManager(
        bot_data.trade_guardian.handle_ticker_update
    )
    ...
    bot_data.guardian_listener_task = asyncio.create_task(
        bot_data.trade_guardian.command_listener()
    )

    logger.info(
        "WebSocket engines and Command Listener started. Waiting 5s for connections..."
    )
    await asyncio.sleep(5)

    # الاشتراك في تحديثات الأسعار للصفقات المحملة في الذاكرة
    await bot_data.public_ws.subscribe(
        [symbol for symbol in bot_data.active_trades_cache.keys()]
    )

    # --- 6. جدولة المهام الدورية ---
    ...
    jq.run_repeating(
        ...
        first=30,
        name="the_supervisor_job",
    )
    jq.run_repeating(
        wise_man.intelligent_reviewer_job,
        interval=1800,
        first=300,
        name="intelligent_reviewer_job",
    )
    jq.run_repeating(
        wise_man.review_active_trades_with_tactics,
        interval=900,
        first=120,
        name="wise_man_tactical_review",
    )
    # (بقية مهامك القديمة مثل تدريب النموذج ومراجعة المحفظة)
    jq.run_repeating(
        wise_man.review_portfolio_risk,
        interval=3600,
        first=90,
        name="wise_man_portfolio_review",
    )
    jq.run_repeating(
        wise_man.train_ml_model, interval=604800, first=3600, name="wise_man_ml_train"
    )

    logger.info(f"All Focused jobs scheduled. OKX Bot is fully operational.")
    await application.bot.send_message(
        TELEGRAM_CHAT_ID, "*🤖 بوت OKX v9.3 (النسخة المركزة) - بدأ العمل...*"
    )


async def load_active_trades_to_cache():
    """[دالة جديدة ومهمة] تقوم بتحميل الصفقات النشطة من قاعدة البيانات إلى الذاكرة عند بدء التشغيل."""
    async with aiosqlite.connect(DB_FILE) as conn:
        conn.row_factory = aiosqlite.Row
        active_trades = await (
            await conn.execute("SELECT * FROM trades WHERE status = 'active'")
        ).fetchall()
        for trade in active_trades:
            trade_dict = dict(trade)
            bot_data.active_trades_cache[trade_dict["symbol"]] = trade_dict
        logger.info(
            f"Loaded {len(active_trades)} active trades into the in-memory cache."
        )
