# -*- coding: utf-8 -*-
# =======================================================================================
# --- 🎨 ملف واجهة المستخدم الكامل والنهائي v5.0 (الترتيب الصحيح) 🎨 ---
# =======================================================================================
# تم إعادة ترتيب الدوال لحل مشكلة NameError.
...
def determine_active_preset_name(settings):
    for name, preset_settings in SETTINGS_PRESETS.items():
        ...
        if is_match:
            from settings_config import PRESET_NAMES_AR # Lazy import to avoid circular dependency
            return PRESET_NAMES_AR.get(name, name)
    return "مخصص"

# =======================================================================================
# --- دوال عرض لوحة التحكم والإعدادات (تم وضعها هنا ليتم تعريفها أولاً) ---
# =======================================================================================
...
async def show_trades_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not trades:
        ...
        keyboard = [[InlineKeyboardButton(f"#{t['id']} {'✅' if t['status'] == 'active' else '⏳'} | {t['symbol']}", callback_data=f"check_{t['id']}")] for t in trades]
...
async def check_trade_details(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    trade = dict(trade)
    keyboard = [[InlineKeyboardButton("🚨 بيع فوري", callback_data=f"manual_sell_confirm_{trade_id}")], [InlineKeyboardButton("🔙 العودة للصفقات", callback_data="db_trades")]]
    if trade['status'] == 'pending':
        ...
        try:
            ...
            pnl_text = f"💰 **الربح/الخسارة:** `${pnl:+.2f}` ({pnl_percent:+.2f}%)"
...
async def show_trade_history_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    async with aiosqlite.connect(DB_FILE) as conn:
        conn.row_factory = aiosqlite.Row
        closed_trades = await (await conn.execute("SELECT symbol, pnl_usdt FROM trades WHERE status NOT IN ('active', 'pending', 'closing') ORDER BY id DESC LIMIT 10")).fetchall()
...
async def show_stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    async with aiosqlite.connect(DB_FILE) as conn:
        conn.row_factory = aiosqlite.Row
        trades_data = await (await conn.execute("SELECT pnl_usdt FROM trades WHERE pnl_usdt IS NOT NULL")).fetchall()
    if not trades_data: text = "لم يتم إغلاق أي صفقات بعد."
    else:
        pnls = [t['pnl_usdt'] for t in trades_data]
        total_pnl = sum(pnls)
        wins_data = [p for p in pnls if p >= 0]
        losses_data = [p for p in pnls if p < 0]
        win_rate = (len(wins_data) / len(pnls) * 100) if pnls else 0
...
async def show_strategy_report_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not performance: text = "لا توجد بيانات أداء حاليًا."
    else:
...
async def show_diagnostics_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    bot_data = context.bot_data; scan_info = bot_data.last_scan_info
...
async def show_parameters_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    s = context.bot_data.settings
    def bool_format(key): return "✅" if s.get(key, False) else "❌"
    keyboard = [[InlineKeyboardButton(f"حجم الصفقة ($): {s['real_trade_size_usdt']}", callback_data="param_set_real_trade_size_usdt")], [InlineKeyboardButton(f"أقصى عدد للصفقات: {s['max_concurrent_trades']}", callback_data="param_set_max_concurrent_trades")], [InlineKeyboardButton(f"مضاعف وقف الخسارة (ATR): {s['atr_sl_multiplier']}", callback_data="param_set_atr_sl_multiplier")], [InlineKeyboardButton(f"نسبة المخاطرة/العائد: {s['risk_reward_ratio']}", callback_data="param_set_risk_reward_ratio")], [InlineKeyboardButton(f"الوقف المتحرك: {bool_format('trailing_sl_enabled')}", callback_data="param_toggle_trailing_sl_enabled")], [InlineKeyboardButton("🔙 العودة للإعدادات", callback_data="settings_main")]]
...
async def handle_setting_value(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'blacklist_action' in context.user_data:
        ...
        try:
            ...
            # This is a simplified way, needs proper nested handling for complex keys
        ...
        finally: del context.user_data['setting_to_change']
    with open(SETTINGS_FILE, 'w') as f: import json; json.dump(settings, f, indent=4)
